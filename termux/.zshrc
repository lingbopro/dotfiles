...
zinit ice lucid wait=1 ascompletion && zinit snippet /data/data/com.termux/files/home/.local/share/tmoe-linux/git/share/old-version/share/completion/_tmoe
...
# thanks p10k
export GPG_TTY="$_P9K_SSH_TTY"

# Acode Language Server
alsp() {
	if ! ps aux | grep -E 'node .+/node_modules/acode-lsp/.+js' | grep -v grep >/dev/null; then
		nohup acode-ls >~/.cache/acode-ls.log 2>&1 &
	else
		return 1
	fi
}
